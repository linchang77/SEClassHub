...
***************************************************************************/
void cls(const HANDLE hout)//按q之后，清屏然后再调menu 
{
	COORD coord = { 0, 0 };
...
void init_border(const HANDLE hout)
{
	...
	/* 随机显示20个大写字母，字母的值、XY坐标都随机显示
	   ...
	   思考：在什么情况下，下面这个循环执行生成后，你看到的实际字母个数不足20个？ *///随机到一样的数字了
...
int menu(const HANDLE hout)
{
	...
	cout << "5.用I、J、K、L键控制上下左右(大小写均可，边界停止,且左方向键不会移动)" << endl;
	cout << "6.用I、J、K、L键控制上下左右(大小写均可，边界回绕，且左方向键不会移动)" << endl;
	cout << "0.退出" << endl;
	cout << "[请选择0-6]";
	while (1)
	{
