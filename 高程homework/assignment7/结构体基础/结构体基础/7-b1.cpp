...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
int getmonth(int &totaldays, int year)
{
	...
	while (totaldays>31)
	{
		totaldays -= days[(year % 4 == 0 && year % 100 != 0) || (year % 400 == 0)][month];
		month++;
		if (totaldays < days[(year % 4 == 0 && year % 100 != 0) || (year % 400 == 0)][month])
			return month+1;
	}
	return 1;
}
...
struct tj_time* tj_time_convert(int input_time)
{
	...
	int totaldays = input_time / 60 / 60 / 24+1;
	result.tj_year = getyear(totaldays);
	result.tj_month = getmonth(totaldays,result.tj_year);
	result.tj_day = totaldays;
